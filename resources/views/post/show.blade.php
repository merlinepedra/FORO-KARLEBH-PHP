  ...
  <div class="mx-auto w-full bg-gray-50 rounded-md p-6 dark:bg-gray-700 dark:text-gray-300">
    ...
    <div>
      ...
      <div class="flex items-center mt-2">
        ...
        <Like :likeable_id="{{ $post->id }}" likeable_type="{{ $post::class }}" :user_id="{{ $post->user_id }}" class="mr-10"></Like>

        @can('update', $post)
        <a href="{{ route('post.edit', $post) }}" class="font-semibold textlg md:text-2xl mr-10">
      ...
      <div class="border-2 rounded-md p-6 mb-4">
        ...
        <div>
          <div class="mt-5 flex items-center">
            <Like :likeable_id="{{ $comment->id }}" likeable_type="{{ $comment::class }}" :user_id="{{ $comment->user_id }}" class="mr-8"></Like>

            {{-- <a href="#top" data-parent-id="{{ $comment->id }}" v-on:click="reply" class="scrollLinks text-gray-800 font-semibold text-2xl">Reply</a> --}}
            @can('reply', $comment)
            <div class="flex mr-10">
              <a href="{{ route('reply.create', $comment) }}" data-parent-id="{{ $comment->id }}" id="replyBtn" class="text-gray-800 font-semibold text-2xl">
          ...
          <div class="border-2 rounded-md p-6 mt-6">
            ...
            <div class="inline-flex">
              <Like :likeable_id="{{ $reply->id }}" likeable_type="{{ $reply::class }}" :user_id="{{ $reply->user_id }}" class="mr-10"></Like>
