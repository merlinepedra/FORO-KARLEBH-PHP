...
<div class="flex justify-between items-center">
  ...
  <div class="w-5/12 flex items-center bg-gray-200 rounded-l-sm pl-3 py-2">
   ...
   <input placeholder="Search for topics" class="w-full bg-gray-200 rounded-r-sm focus:outline-none">
 </div>
  ...
  <span class="mr-8">
    ...
    <div class="relative flex-shrink-0">
      ...
      <transition name="flip">
        <div v-if="profileMenu" id="profile-menu" class="font-light absolute right-0 bg-gray-100 pt-4 mt-5 z-10 w-48 text-center shadow-md">
          <a v-on:click="profileMenu = false" class="block mt-3 hover:bg-blue-100 px-2 py-2" href="{{ route('profile.edit', auth()->user()->profile->name) }}">Edit Profile</a>
          <a v-on:click="profileMenu = false" class="block mt-3 hover:bg-blue-100 px-2 py-2" href="{{ route('profile.show', auth()->user()->profile->name) }}">
            {{-- View Profile --}}
            {{ auth()->user()->name }}
          </a>
            <form action="{{ route('logout') }}" id="logout" method="POST">
              @csrf
            <button v-on:click="profileMenu = false" class="w-full hover:bg-blue-100 py-2 mt-3" href="">Logout</button>
            </form>
          </div>
        </transition>
        @else
