    ...
    <div class="md:hidden relative">
      <form action="flex">
        <div class="flex-1 flex items-center px-3 py-2">
          <input type="text" v-model="item" @input="search" placeholder="Search for topics" class="w-full rounded-md bg-gray-200 focus:ring-0 focus:border-blue-900">
    ...
    <div class="hidden md:block relative bg-gray-200">
      <form action="flex">
        <div class="flex-1 flex px-3 py-2">
          ...
          <input placeholder="Search for topics" v-model="item" @input="search" class="flex-1 bg-gray-200 rounded-r-sm focus:outline-none">
          <button v-if="item.length > 0" @click.prevent="clearInput">
            <svg class="h-6 w-6" stroke="currentColor" fill="none" viewBox="0 0 24 24">
              <path class="inline-flex" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
            </svg>
