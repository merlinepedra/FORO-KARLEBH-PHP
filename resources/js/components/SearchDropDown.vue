    ...
    <div class="hidden md:block relative bg-gray-200 dark:bg-gray-400">
      <form action="flex">
        <div class="flex-1 flex px-3 py-2 rounded-lg overflow-hidden">
          ...
          <input placeholder="Search for topics" v-model="item" @input="searcher" 
          class="flex-1 bg-gray-200 dark:bg-gray-400 dark:text-gray-800 rounded-r-sm focus:outline-none" readonly>
          <button v-if="item.length > 0" @click.prevent="clearInput" >
      ...
      <div class="absolute z-10 rounded-md shadow-md mt-3 w-full bg-gray-200 dark:bg-gray-400 dark:text-gray-800" v-if="results.length > 0">
        <div v-for="result in results" :key="result.id" class="px-3 py-2 hover:bg-blue-100">
          <a :href="getUrl(result.slug)">{{ result.title }}   </a>
        </div>  
      </div>
